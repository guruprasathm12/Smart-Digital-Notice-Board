...
$sql=mysqli_query($conn,"select * from time_slide where board='b3'");
...
{
	include "config.php";
	$que=mysqli_query($conn,"select * from tbl_images where board='b3' and id=$id");
...
<?php
$aa=mysqli_query($conn,"select * from tbl_images where board='b3'");
$a1=mysqli_num_rows($aa);

if(intval($idd)==intval($a1))
{
	mysqli_query($conn,"update time_slide set id=1 where board='b3'");
}
else
{
	$idd=$idd+1;
	mysqli_query($conn,"update time_slide set id=$idd where board='b3'");
}
rsort($l);
?>
...
function fun()
{
	...
	setTimeout("location.href='b3.php'",dur+"000");
}
